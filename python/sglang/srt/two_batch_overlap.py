...
class AfdForwardBatchPreparer:
    ...
    def prepare(cls, batch: ForwardBatch):
        # cannot be used together with tbo

        if (
            batch.afd_split_seq_index is None
            or get_afd_mirco_batch() is None
            or get_afd_mirco_batch() == 1
        ):
            return
    ...
    def prepare_raw(
        ...
        if m == 2:
            ...
            assert isinstance(batch.attn_backend, AfdAttnBackend)
            attn_backend_child_a, attn_backend_child_b, attn_backend_child_c = (
                batch.attn_backend.children
            )

            [
                out_num_token_non_padded_a,
                out_num_token_non_padded_b,
                out_num_token_non_padded_c,
            ] = afd_children_num_token_non_padded
