        ...
        def prepare_overlap(batch: ScheduleBatch):
            ...
            if forward_mode == ForwardMode.EXTEND:
                ...
                if m == 2:
                    ...
                    batch.tbo_split_seq_index = [
                        tbo_split_seq_index,
                        2 * tbo_split_seq_index,
                    ]
            else:
                raise NotImplementedError()

        logger.info(
            "event_loop_afd: role={} m={}".format(
                get_afd_perspective(), get_afd_mirco_batch()
            )
        )
        while True:
            ...
            if batch:
                if afd_is_attn() and self.pp_rank == 0 and self.attn_tp_rank == 0:
                    afd_req = AFDReqInput(batch.batch_size(), batch.forward_mode)
        ...
        if new_batch is not None:
            ...
            if not self.running_batch.is_empty():
                if afd_is_ffn():
                    ...
                    assert (
                        batch.batch_size() >= self.afd_batchsize_attn
                    ), "{} < {}".format(batch.batch_size(), self.afd_batchsize_attn)
